				...
				using (StreamReader reader = new StreamReader(stream))
				{
					...
					while (reader.ReadLine() != null && counter <= 700)
					{
						...
						for (int i = 0; i < itemTypeCount; i++)
						{
							...
							Item item = new Item("0", Convert.ToDecimal(itemArray[1]), Convert.ToDecimal(itemArray[3]), Convert.ToDecimal(itemArray[5]), Convert.ToInt32(itemArray[7]), true, 'C', false);
						...
						List<ContainerPackingResult> result = PackingService.Pack(containers, itemsToPack, new List<int> { (int)AlgorithmType.EbAfit });
